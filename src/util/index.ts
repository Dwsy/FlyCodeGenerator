...
export const addButton = (
  ...
  if (
    document.querySelector(
      `#beSetting > div.main-content > div.tab-operation > button:nth-child(${2 + ButtonSeq()})`
    ) != null
  ) {
    return
  }
...
export function toCamelCase(str: string): string {
  ...
  return str.replace(/_([a-z])/g, (match, p1) => p1.toUpperCase()).replace(/^([a-z])/g, (match, p1) => p1.toUpperCase())
}
