...
import {
  excelFrontImportTemplate,
  objDocDefTempalte,
  paramDocTempalte,
  propertyDocTempalte
} from '../../data/excelFrontImportTemplate'
...
export const generateCodeFunc = (mapPair: MapPair[], checkedRowKeys: RowKey[]) => {
  ...
  //     .replaceAll("{", "       {")
  code = code
    .concat(templet.ExcelImpSettingFunc)
  ...
  checkMapPair
    ...
    .forEach((pair) => {
      ...
      let callFunc =
        ...
        `    rowData.${pair.property.name} =\n        get${toCamelCase(dictTableName)}DictIdByDicvalue(rowData.${
          pair.field
        },index)`
  ...
  checkMapPair
    ...
    .filter((pair) => {
      return (
        ...
        Number(pair.property.propertytypecode) == PropertyTypeCode.LongInteger
      )
    })
    .forEach((pair) => {
      ...
      let getBusinessObjectIdByValue = templet.getBusinessObjectIdByValue

      getBusinessObjectIdByValue = getBusinessObjectIdByValue
        .replaceAll(
          '{{required}}',
          templet.requiredCode.replace('{{data}}', pair.reverseQueryField).replace('{{text}}', pair.field)
        )
        ...
        .replaceAll('{{tableName}}', tableName)
        .replaceAll('{{BusinessObjectTableZhName}}', pair.property.propertyname)

      getBusinessObjectIdByValueArray.push(getBusinessObjectIdByValue)
      let callFunc =
        `//反查对象 ${pair.property.propertyname}\n` +
        `rowData.${pair.property.name} =\n        get${toCamelCase(tableName)}IdBy${toCamelCase(
          pair.reverseQueryField
        )}(rowData.${pair.field},index)`
      if (pair.property.name !== pair.field) {
        callFunc += `\n delete rowData.${pair.field}`
      }
  ...
  const callReverseQueryFuncs = callGetDictIdByDicvalueArray
    .concat(callGetBusinessObjectIdByValueArray)
    .join('\n\n    ')
  ...
  const genParamDocArray = () => {
    return checkMapPair.map((pair) => {
      ...
      return paramDocTempalte.replace('{{paramName}}', data.paramName).replace('{{paramDesc}}', data.paramDesc)
    })
  }
  const genPropertyDocArray = () => {
    return checkMapPair.map((pair) => {
      ...
      return propertyDocTempalte.replace('{{propertyName}}', data.paramName).replace('{{propertyDesc}}', data.paramDesc)
    })
  }
  const inputRowDataJsDoc = objDocDefTempalte
    .replace('{{desc}}', 'Excel行数据')
    .replace('{{objctName}}', 'inputRowData')
    .replace('{{propertyDocArray}}', genPropertyDocArray().join('\n'))

  const outRowDataJsDoc = objDocDefTempalte
    .replace('{{desc}}', tableName + '对象')
...
export const autoMapFunc = (excelColumnName: string[], sheetLine: []) => {
  ...
  //   let temp = flyStore.tableDataMap.get(flyStore.protocol.input[0].objectcode).properties
  //   let temp2 = [...temp]
  //   temp
  //   .filter(d => get)
  //   .forEach((property) => {
